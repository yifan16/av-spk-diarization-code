...
import pdb
import time
from utils.tools import calc_pdist,calc_pdist_npy
import numpy
...
class S(nn.Module):
    def __init__(self, num_layers_in_fc_layers = 1024):
        ...
        self.netcnnlip = nn.Sequential(
            nn.Conv3d(1, 96, kernel_size=(5,7,7), stride=(1,2,2), padding=0),
            nn.BatchNorm3d(96),
            nn.ReLU(inplace=True),
            ...
            nn.Conv3d(256, 512, kernel_size=(1,2,2), padding=0),
            nn.BatchNorm3d(512),
            nn.ReLU(inplace=True),
        );
    ...
    def evaluate(self, opt, imtv,cct):
        ...
        for i in range(0,lastframe,opt.batch_size):
            ...
            im_out  = self.forward_lipfeat(im_in.cuda());
        ...
        dists = calc_pdist_npy(im_feat,cc_feat.numpy(),vshift=opt.vshift)
    ...
    def evaluate_allcase(self, opt, imtv,cct,cct_neg):
        ...
        for i in range(0,lastframe,opt.batch_size):
            ...
            cc_batch = [ cct_neg[:,:,:,vframe*4:vframe*4+20] for vframe in range(i,min(lastframe,i+opt.batch_size)) ]
            cc_in = torch.cat(cc_batch,0)
            cc_out  = self.forward_aud(cc_in.cuda())
            cc_neg_feat.append(cc_out.data.cpu())

        im_feat = torch.cat(im_feat,0)
        cc_feat = torch.cat(cc_feat,0)
        cc_neg_feat = torch.cat(cc_neg_feat,0)
        ...
        dists = calc_pdist_npy(im_feat,cc_feat.numpy(),vshift=opt.vshift)
        dists_neg = calc_pdist_npy(im_feat,cc_neg_feat.numpy(),vshift=opt.vshift)
        #dists = calc_pdist(im_feat,cc_feat,vshift=opt.vshift)
        #print('shape',len(dists))
        #print(dists[0].shape)
        mdist = torch.mean(torch.stack(dists,1),1)
        mdist_neg = torch.mean(torch.stack(dists_neg,1),1)

        print('mdist',mdist)

        minval, minidx = torch.min(mdist,0)
        minval_neg, minidx_neg = torch.min(mdist_neg,0)

        offset = opt.vshift-minidx
        offset_neg = opt.vshift-minidx_neg
        conf   = torch.median(mdist) - minval
        ...
        print('Framewise conf: ')
        print(fconfm)
        print('Neg offset: \t%d \nAV offset: \t%d \nMin dist: \t%.3f\nConfidence: \t%.3f' % (offset_neg,offset,minval,conf))

        dists_npy = numpy.array([ dist.numpy() for dist in dists ])
        return offset.numpy(), conf.numpy(), dists_npy, minval.numpy(),offset_neg.numpy()
    ...
    def tsne_feature(self,opt, imtv,cct):
        ...
        im_feat = im_feat.numpy()
        cc_feat = cc_feat.numpy()
